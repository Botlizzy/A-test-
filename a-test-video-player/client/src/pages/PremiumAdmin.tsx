...
export default function PremiumAdmin({ user, onBack, onSignOut }: PremiumAdminProps) {
  ...
  const activateRequest = async (request: VerificationRequest) => {
    ...
    else { setRequests((items) => items.map((item) => item.id === request.id ? { ...item, status: "activated", reviewed_at: auditTime, reviewed_by: user.email } : item)); setMessage("Premium access activated from the verification request."); }
  };
  ...
  const updatePremium = async (active: boolean) => {
    ...
    if (updateError) setError(updateError.message.includes("premium_entitlements") ? "The premium table is not ready. Run supabase/schema.sql first." : updateError.message);
      ...
      if (selectedRequestId) {
        const { error: requestUpdateError } = await supabase.from("verification_requests").update({ status: active ? "activated" : "declined", transaction_reference: reference.trim() || null, reviewed_at: auditTime, reviewed_by: user.email || null }).eq("id", selectedRequestId);
        if (requestUpdateError) setError(requestUpdateError.message);
        else setRequests((items) => items.map((item) => item.id === selectedRequestId ? { ...item, status: active ? "activated" : "declined", transaction_reference: reference.trim() || null, reviewed_at: auditTime, reviewed_by: user.email } : item));
      }
