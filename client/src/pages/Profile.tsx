...
export default function Profile({ user, onBack, onSignOut }: ProfileProps) {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      const [{ data, error: profileError }, { data: entitlement }] = await Promise.all([
        ...
        supabase.from("premium_entitlements").select("active, activated_by, activated_at, expires_at").eq("user_id", user.id).maybeSingle(),
      ]);
      if (!active) return;
      if (profileError) {
      ...
      const nextPremiumStatus: PremiumStatus = ownerPremium || entitlementActive ? "active" : entitlement ? (entitlement.activated_by ? "inactive" : "pending") : "inactive";
      setPremiumStatus(nextPremiumStatus);
      setPremiumActivatedAt(entitlementActive && entitlement?.activated_at ? entitlement.activated_at : null);
      setPremiumExpiresAt(entitlementActive && entitlement?.expires_at ? entitlement.expires_at : null);
      setLoading(false);
    };
    void loadProfile();
    return () => { active = false; };
  ...
  return <div className="profile-shell"><header className="profile-topbar"><a className="brand" href="#top" onClick={(event) => { event.preventDefault(); onBack(); }}><span className="brand-mark"><span className="signal-mark"><span /><span /><span /></span></span><span><strong>ELIZZY</strong><em>DOMAIN</em></span></a><div className="profile-topbar__actions"><button className="profile-link" onClick={onBack}><ArrowLeft size={15} /> Back to feed</button><a className="profile-link profile-link--muted" href="mailto:elijahchinecheremonah@gmail.com?subject=ELIZZY%20DOMAIN%20feedback">Feedback</a><button className="profile-link profile-link--muted" onClick={onSignOut}><LogOut size={15} /> Sign out</button></div></header>
