...
type ToolHubProps = { user?: User; onBack: () => void; onSignOut: () => Promise<void>; embedded?: boolean; publicInfoOnly?: boolean; hiddenCategories?: string[]; hiddenPaths?: string[] };
...
function apiRequestSnippet(endpoint: Endpoint, base: string, fields: string[]) { const url = `${base}${endpoint.path}`; const sample = fields.filter((field) => field !== "sessionId").map((field) => `${field}=YOUR_${field.toUpperCase()}`).join("&"); return endpoint.method.includes("POST") ? `curl -X POST "${url}" \\\n  -H "Content-Type: application/json" \\\n  -d '{"${fields.filter((field) => field !== "sessionId").join('":"YOUR_VALUE","')}"${fields.length ? '":"YOUR_VALUE"' : ""}}'` : `curl "${url}${sample ? `?${sample}` : ""}"`; }
...
export default function ToolHub({ user, onBack, onSignOut, embedded = false, publicInfoOnly = false, hiddenCategories = [], hiddenPaths = [] }: ToolHubProps) {
  ...
  const visibleEndpoints = useMemo(() => catalog.endpoints.filter((item) => !hiddenCategories.includes(item.category) && !hiddenPaths.includes(item.path)), [hiddenCategories, hiddenPaths]);
  const categories = useMemo(() => Array.from(new Set(visibleEndpoints.map((item) => item.category))), [visibleEndpoints]);
  const filtered = useMemo(() => visibleEndpoints.filter((item) => (category === "all" || item.category === category) && `${item.alias} ${item.path} ${item.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase())), [category, query, visibleEndpoints]);
  const selected = visibleEndpoints.find((item) => item.path === selectedPath) || null;
  ...
  const videoLinks = selected?.category === "download" ? links.filter((link) => !isImageUrl(link) && !isAudioUrl(link)) : links.filter(isVideoUrl);
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); if (!publicInfoOnly && params.get("run") === "1" && selected && !isAdult(selected)) void execute(); }, [selectedPath, publicInfoOnly]);
  ...
  if (selected && isAdult(selected) && !adultAccepted) return <div className={`tools-shell ${embedded ? "tools-shell--embedded" : ""}`}><header className={`tools-topbar ${embedded ? "tools-topbar--embedded" : ""}`}><button className="brand" onClick={onBack}><span className="brand-mark"><span className="signal-mark"><span /><span /><span /></span></span><span><strong>eliminator</strong><em>multitools</em></span></button></header><main className="tools-gate"><div className="tools-gate__card"><span className="tools-gate__icon"><ShieldCheck size={26} /></span><span className="eyebrow eyebrow--red">18+ TOOL SPACE</span><h1>Confirm before continuing.</h1><p>This endpoint can return adult material. Continue only if you are legally an adult where you are.</p><button className="red-button" onClick={() => { localStorage.setItem("eliminator-tools-18-plus", "true"); setAdultAccepted(true); }}>I’m 18+ — open tool</button><button className="secondary-button" onClick={() => setSelectedPath("")}>Return to tools</button></div></main></div>;

  return <div className={`tools-shell ${embedded ? "tools-shell--embedded" : ""}`}><header className={`tools-topbar ${embedded ? "tools-topbar--embedded" : ""}`}><button className="brand" onClick={onBack}><span className="brand-mark"><span className="signal-mark"><span /><span /><span /></span></span><span><strong>eliminator</strong><em>multitools</em></span></button><div className="tools-topbar__actions"><span className="tools-build" data-build-revision={liveRevision}>Build {liveRevision}</span><span className="tools-user">{user ? (user.user_metadata?.full_name || user.email) : "Public tools"}</span><button className="refresh-button" onClick={onBack}><ArrowLeft size={15} /> Feed</button>{user && <button className="refresh-button" onClick={onSignOut}><X size={15} /> Sign out</button>}</div></header><main className="tools-layout"><aside className="tools-sidebar"><div className="tools-sidebar__intro"><span className="eyebrow eyebrow--blue">ELIMINATOR LAB</span><h1>Every endpoint,<br /><i>one workspace.</i></h1><p>{visibleEndpoints.length} public tools across {categories.length} task spaces. Choose what you want to do, not the API behind it.</p></div><label className="tools-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tasks" /></label><div className="tools-featured"><span className="eyebrow eyebrow--red">START WITH A TASK</span>{featuredTasks.map((task) => <button key={task.path} onClick={() => { const endpoint = visibleEndpoints.find((item) => item.path === task.path); if (endpoint) choose(endpoint); }}>{task.label}</button>)}</div><div className="tools-categories"><button className={category === "all" ? "is-active" : ""} onClick={() => setCategory("all")}>All public tools <b>{visibleEndpoints.length}</b></button>{categories.map((item) => <button className={category === item ? "is-active" : ""} key={item} onClick={() => setCategory(item)}>{prettyCategory(item)} <b>{visibleEndpoints.filter((endpoint) => endpoint.category === item).length}</b></button>)}</div></aside><section className="tools-content"><div className="tools-heading"><div><span className="eyebrow">{category === "all" ? "ALL ENDPOINTS" : prettyCategory(category).toUpperCase()}</span><h2>{selected ? selected.alias : "Choose a tool to begin."}</h2><p>{selected ? taskDescription(selected) : "Choose a task and complete it here with a focused input, a real result, and save/copy actions."}</p></div><span className="tools-count">{filtered.length} shown</span></div><div className="tools-body"><div className="tools-grid">{filtered.map((endpoint) => <button className={`tool-card ${selectedPath === endpoint.path ? "is-selected" : ""}`} key={`${endpoint.category}-${endpoint.path}`} onClick={() => choose(endpoint)}><span className="tool-card__icon">{isAdult(endpoint) ? <ShieldCheck size={17} /> : endpoint.category === "download" ? <Download size={17} /> : endpoint.category === "ai" ? <Sparkles size={17} /> : <Wrench size={17} />}</span><span><strong>{endpoint.alias}</strong><small>{taskTitle(endpoint)}</small></span><span className="tool-card__method">{functionalMode(endpoint) === "downloader" ? "DO IT" : functionalMode(endpoint) === "ai" || functionalMode(endpoint) === "media" ? "MAKE" : "RUN"}</span></button>)}</div>{selected && publicInfoOnly && <section className="tool-workspace tool-api-info"><div className="tool-workspace__top"><div><span className="eyebrow eyebrow--red">PUBLIC API REFERENCE</span><h3>{selected.alias}</h3><p>{taskDescription(selected)} The homepage does not execute this request; use Premium Room for the functional workspace.</p></div><button className="text-button" onClick={() => { setSelectedPath(""); setResult(null); }}>Close details</button></div><div className="tool-api-info__meta"><span><b>Method</b>{selected.method}</span><span><b>Category</b>{prettyCategory(selected.category)}</span><span><b>Path</b>{selected.path}</span></div><div className="tool-api-info__section"><div className="tool-api-info__section-head"><span className="eyebrow eyebrow--blue">COPYABLE REQUEST</span><button className="primary-button" onClick={() => { navigator.clipboard?.writeText(apiRequestSnippet(selected, catalog.base, activeFields)); setCopied(true); window.setTimeout(() => setCopied(false), 1400); }}><Copy size={15} /> {copied ? "API copied" : "Copy API"}</button></div><pre className="tool-api-info__code">{apiRequestSnippet(selected, catalog.base, activeFields)}</pre></div><div className="tool-api-info__section"><span className="eyebrow eyebrow--blue">DOCUMENTATION</span><p className="tool-api-info__description">{selected.text || taskDescription(selected)}</p><div className="tool-api-info__params">{activeFields.map((field) => <span key={field}>{fieldLabel(field, activeMode)}</span>)}</div></div></section>}{selected && !publicInfoOnly && <section className="tool-workspace"><div className="tool-workspace__top"><div><span className="eyebrow eyebrow--blue">{modeLabel(activeMode)}</span><h3>{taskTitle(selected)}</h3><p>{taskDescription(selected)}</p></div><button className="text-button" onClick={() => { setSelectedPath(""); setResult(null); }}>Close workspace</button></div><div className="tool-form"><div className="tool-fields">{activeFields.map((field) => <label key={field}>{fieldLabel(field, activeMode)}<input value={fields[field] || ""} onChange={(event) => updateField(field, event.target.value)} placeholder={field === "url" ? "https://…" : field === "prompt" ? "Describe the result you want…" : field === "q" || field === "query" ? "Type your search…" : `Enter ${field}`} /></label>)}</div><p className="tool-hint">Need something extra? Add optional settings below. You never need to know the service URL to use this task.</p><label className="tool-json-field">Advanced options (optional)<textarea value={fields.__json || ""} onChange={(event) => updateField("__json", event.target.value)} placeholder='{"option":"value"}' rows={4} /></label><button className="primary-button" onClick={execute} disabled={busy}>{busy ? <LoaderCircle size={16} className="spin" /> : <Sparkles size={16} />} {busy ? "Working on it…" : activeMode === "downloader" ? "Get my file" : activeMode === "ai" || activeMode === "media" ? "Create result" : "Complete task"}</button></div>{error && <div className="tools-error">{error}</div>}{result !== null && <div className="tool-result"><div className="tool-result__head"><span className="eyebrow">RESULT</span><div><button className="text-button" onClick={() => { navigator.clipboard?.writeText(stringify(result)); setCopied(true); window.setTimeout(() => setCopied(false), 1200); }}><Copy size={14} /> {copied ? "Copied" : "Copy details"}</button></div></div>{links.length > 0 && <div className="tool-result__links">{links.map((link) => <div className="tool-result__link-row" key={link}><a href={link} target="_blank" rel="noreferrer">{videoLinks.includes(link) ? "Open video preview" : isImageUrl(link) ? "Open image result" : "Open returned file"}</a><button className="tool-download" onClick={() => downloadLink(link)} disabled={downloadState[link] === "downloading"}>{downloadState[link] === "downloading" ? <LoaderCircle size={13} className="spin" /> : <Download size={13} />} {downloadState[link] === "downloading" ? "Preparing…" : downloadState[link] === "success" ? "Saved" : downloadState[link] === "error" ? "Retry" : "Save"}</button></div>)}</div>}{videoLinks.length > 0 && <div className="tool-result__media">{videoLinks.map((link) => <video key={link} controls preload="metadata" src={link} aria-label="Returned video preview" />)}</div>}{selected?.category === "download" && result !== null && videoLinks.length === 0 && <p className="tool-result__note">The endpoint returned a response but did not include a direct media file URL. Try another source link or endpoint.</p>}{links.some(isImageUrl) && <div className="tool-result__media">{links.filter(isImageUrl).map((link) => <img key={link} src={link} alt="Returned result" loading="lazy" />)}</div>}{audioLinks.length > 0 && <div className="tool-result__audio"><span className="eyebrow eyebrow--blue">FULL TRACK RETURNED</span>{audioLinks.map((link) => <div className="tool-result__audio-row" key={link}><audio controls preload="metadata" src={link} aria-label="Complete returned track" /><button className="tool-download" onClick={() => downloadLink(link)} disabled={downloadState[link] === "downloading"}>{downloadState[link] === "downloading" ? "Preparing…" : downloadState[link] === "success" ? "Saved" : downloadState[link] === "error" ? "Retry save" : "Save full track"}</button></div>)}</div>}{selected.category === "aimusic" && audioLinks.length === 0 && <p className="tool-result__note">The endpoint returned its JSON response, but no complete audio URL was included yet. For async Suno jobs, copy the task ID into Suno Status and run it again when the track is ready.</p>}<details className="tool-result__details"><summary>View technical response</summary><pre>{stringify(result)}</pre></details></div>}</section>}</div></section></main></div>;
}
