...
const isProtectedAdminEmail = isApprovedAdminEmail;
...
export default function PremiumAdmin({ user, onBack, onSignOut }: PremiumAdminProps) {
  ...
  const deleteMemberAccount = async (member: Customer) => {
    if (!supabase || !isAdmin || deleteBusyId || isProtectedAdminEmail(member.email)) return;
