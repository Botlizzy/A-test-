...
import { ArrowLeft, Bell, Check, CircleAlert, LoaderCircle, LockKeyhole, LogOut, RefreshCw, Search, ShieldCheck, Trash2, UserRound, X } from "lucide-react";
...
import { getAvatarStoragePath, isDeleteConfirmationValid } from "@/lib/adminAccountDeletion";
...
type Customer = { id: string; full_name: string; email: string; avatar_url?: string | null; account_status?: "active" | "suspended"; account_warning?: boolean; account_warning_started_at?: string | null; premium_active?: boolean; premium_expires_at?: string | null };
...
export default function PremiumAdmin({ user, onBack, onSignOut }: PremiumAdminProps) {
  ...
  const deleteMember = async (member: Customer) => {
    if (!supabase || !isAdmin || deleteBusyId) return;
    const firstConfirm = window.confirm(`Permanently delete ${member.email}? This removes the account, profile, premium access, and verification requests. This cannot be undone.`);
    if (!firstConfirm) return;
    const typedEmail = window.prompt(`Type the member's email to confirm deletion:\n${member.email}`);
    if (!typedEmail || !isDeleteConfirmationValid(typedEmail, member.email)) {
      setError("Deletion cancelled. Type the member's exact email address to confirm.");
      return;
    }
    setError(""); setMessage(""); setDeleteBusyId(member.id);
    const avatarPath = getAvatarStoragePath(member.avatar_url, member.id);
    if (avatarPath) {
      const { error: avatarError } = await supabase.storage.from("avatars").remove([avatarPath]);
      if (avatarError) {
        setError(`The account was not deleted because its avatar could not be removed: ${avatarError.message}`);
    ...
    if (deleteError) {
      ...
      if (customer?.id === member.id) { setCustomer(null); setEntitlement(null); setCustomerId(""); }
      setMessage(`${member.email} was permanently deleted from the website.`);
  ...
  const setMemberWarning = async (member: Customer) => {
    ...
    if (warningError) setError(warningError.message.includes("function") ? "Account warnings are not enabled yet. Run the latest Supabase schema first." : warningError.message);
      ...
      const updated = { ...member, account_warning: nextWarning, account_warning_started_at: nextWarning ? new Date().toISOString() : null } as Customer;
      setMembers((items) => items.map((item) => item.id === member.id ? updated : item));
      if (customer?.id === member.id) setCustomer(updated);
