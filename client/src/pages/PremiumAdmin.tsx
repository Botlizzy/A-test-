...
import { accountStatusLabel, getNextAccountStatus } from "@/lib/accountManagement";
...
type Customer = { id: string; full_name: string; email: string; avatar_url?: string | null; account_status?: "active" | "suspended" };
...
export default function PremiumAdmin({ user, onBack, onSignOut }: PremiumAdminProps) {
  ...
  const loadMembers = async () => {
    ...
    const { data, error: membersError } = await supabase.from("profiles").select("id, full_name, email, avatar_url, account_status").order("created_at", { ascending: false }).limit(100);
  ...
  const findCustomer = async (event: FormEvent) => {
    ...
    const [{ data: profile, error: profileError }, { data: status, error: statusError }] = await Promise.all([
      supabase.from("profiles").select("id, full_name, email, avatar_url, account_status").eq("id", normalizedId).maybeSingle(),
