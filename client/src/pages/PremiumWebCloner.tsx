...
export default function PremiumWebCloner() {
  ...
  return <section className="premium-web-cloner" id="premium-web-cloner">
    ...
    {clone.isPending && <div className="premium-web-cloner__progress" role="status" aria-live="polite" aria-atomic="true"><span className="premium-web-cloner__spinner" aria-hidden="true"><LoaderCircle size={20} className="spin" /></span><div><strong>{CLONE_STATUS_MESSAGES[statusIndex]}</strong><p>Web Cloner is working. Keep this page open while the ZIP is prepared.</p><div className="premium-tool-skeleton__rows" aria-hidden="true"><span className="premium-skeleton" /><span className="premium-skeleton" style={{ width: "62%" }} /></div><div className="premium-download-progress__track premium-download-progress__track--indeterminate" role="progressbar" aria-label="Web Cloner is processing"><span /></div><span className="premium-web-cloner__status-count">Step {statusIndex + 1} of {CLONE_STATUS_MESSAGES.length}</span></div></div>}
