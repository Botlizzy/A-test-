...
export default function PremiumRoom({ user, isPremium, onBack, onPricing, onSignOut }: PremiumRoomProps) {
  ...
  const imageMutation = trpc.image.generate.useMutation({
    onSuccess: ({ url }) => { setGeneratedImageUrl(url); setImageError(""); },
    onError: (error) => { setGeneratedImageUrl(""); setImageError(error.message || "Image generation failed. Try again."); },
  });
  ...
  const generatePremiumImage = (event: FormEvent) => {
    event.preventDefault();
    const prompt = imagePrompt.trim();
    if (prompt.length < 3 || imageMutation.isPending) return;
    setImageError("");
    setGeneratedImageUrl("");
    imageMutation.mutate({ prompt, quality: imageQuality });
  };
