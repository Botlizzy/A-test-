...
type SearchItem = { title: string; url?: string; mediaUrl?: string; thumbnail?: string; duration?: string; views?: string; source: "Xvideos Search" | "Xvideo Direct" | "xHamster Random"; sourceUrl?: string };
...
export function extractSearchItems(payload: unknown, source: SearchItem["source"] = "Xvideos Search"): SearchItem[] {
  ...
  const visit = (value: unknown) => {
    ...
    if (title && (url || thumbnail || mediaUrl)) found.push({ title: title.trim(), url, mediaUrl, thumbnail, duration: typeof row.duration === "string" ? row.duration : undefined, views: typeof row.views === "string" || typeof row.views === "number" ? String(row.views) : undefined, source, sourceUrl: url });
    Object.values(row).forEach((child) => { if (child && typeof child === "object") visit(child); });
  };
  visit(payload);
