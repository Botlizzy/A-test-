...
type SearchItem = { title: string; url?: string; mediaUrl?: string; thumbnail?: string; duration?: string; views?: string };
export function extractDirectMediaUrl(row: Record<string, unknown>): string | undefined {
  const candidates: unknown[] = [row.videoUrl, row.video_url, row.video, row.stream, row.mp4, row.m3u8, row.playback_url, row.download_url, row.mediaUrl, row.media_url, row.preview];
  const thumbnail = row.thumbnail;
  if (thumbnail && typeof thumbnail === "object") candidates.push((thumbnail as Record<string, unknown>).preview, (thumbnail as Record<string, unknown>).video, (thumbnail as Record<string, unknown>).url);
  return candidates.find((entry) => typeof entry === "string" && /^https?:\/\//i.test(entry) && /\.(mp4|m3u8|webm|mov)(?:$|[?#])/i.test(entry)) as string | undefined;
}
...
export function extractSearchItems(payload: unknown): SearchItem[] {
  ...
  const visit = (value: unknown) => {
    ...
    if (title && (url || thumbnail || mediaUrl)) found.push({ title: title.trim(), url, mediaUrl, thumbnail, duration: typeof row.duration === "string" ? row.duration : undefined, views: typeof row.views === "string" || typeof row.views === "number" ? String(row.views) : undefined });
    Object.values(row).forEach((child) => { if (child && typeof child === "object") visit(child); });
  };
  visit(payload);
...
export default function Home({ user, onProfile, onPricing, onPremium, onAdmin, onSignOut }: HomeProps) {
  ...
  const searchXXL = async (event: FormEvent) => {
    ...
    try { const url = new URL(XXL_SEARCH_ENDPOINT); url.searchParams.set("query", query); const response = await fetch(url, { headers: { Accept: "application/json" } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(response.status === 400 ? "XXL search needs a different search phrase. Try a shorter title or keyword." : `XXL search returned HTTP ${response.status}.`); const items = extractSearchItems(payload); if (!items.length) throw new Error("No XXL results were returned for that search."); setSearchResults(items); } catch (cause) { setSearchError(cause instanceof Error ? cause.message : "XXL search could not complete."); } finally { setSearchLoading(false); }
  };
  ...
  return <div className="focused-homepage">
    ...
    <main id="home" className="focused-homepage__main">
      ...
      <section id="homepage-xxl" className="homepage-focus-card homepage-focus-card--search"><div className="homepage-focus-card__heading"><div><span className="eyebrow eyebrow--red">XXL DISCOVERY</span><h2>Search the catalog</h2><p>Search results stay in this page. Open a returned source only when you choose it.</p></div><Search size={24} /></div><form className="homepage-search-form" onSubmit={searchXXL}><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search XXL by title or keyword…" aria-label="Search XXL" /><button className="red-button" type="submit" disabled={searchLoading || searchQuery.trim().length < 2}>{searchLoading ? <><RefreshCw size={16} className="spin" />Searching…</> : <><Search size={16} />Search XXL</>}</button></form>{searchError && <div className="homepage-error"><CircleAlert size={17} /><span>{searchError}</span></div>}{searchLoading && <div className="homepage-loading"><RefreshCw size={18} className="spin" /><span>Preparing search results…</span></div>}{searchResults.length > 0 && <div className="homepage-search-results">{searchResults.map((item) => <article className="homepage-search-result" key={`${item.title}-${item.url || item.mediaUrl}`}><div>{item.thumbnail ? <img src={item.thumbnail} alt="" loading="lazy" /> : <span className="homepage-search-result__fallback"><Search size={18} /></span>}</div><section><h3>{item.title}</h3><p>{[item.duration, item.views].filter(Boolean).join(" · ") || "XXL result"}</p>{item.mediaUrl ? <button className="secondary-button" type="button" onClick={() => setSelectedSearchVideo(item)}><Play size={14} />Play inside Eliminator</button> : <span className="homepage-search-result__unavailable">No direct playable file</span>}</section></article>)}</div>}{selectedSearchVideo?.mediaUrl && <div className="homepage-search-player"><div className="homepage-video-player"><video key={selectedSearchVideo.mediaUrl} controls playsInline preload="metadata" poster={selectedSearchVideo.thumbnail} muted={false} src={selectedSearchVideo.mediaUrl} onError={() => setSearchError("This XXL result returned a file the browser could not decode.")} /><div className="homepage-video-player__meta"><div><strong>{selectedSearchVideo.title}</strong><span>{selectedSearchVideo.duration || "XXL direct media"}</span></div><button className="secondary-button" type="button" onClick={() => setSelectedSearchVideo(null)}>Close player</button></div></div></div>}</section>
