export function formatAuthError(rawMessage: string, mode: "login" | "signup"): string {
  ...
  if (/rate limit|too many requests|over_email_send_rate_limit|429/i.test(message)) {
    return "Supabase email delivery is temporarily rate-limited. This is a provider limit, not an ELIZZY DOMAIN signup cooldown. If this email already has an account, use Sign in now; otherwise wait for the provider window to clear or configure a higher-capacity SMTP provider in Supabase. The app does not impose a cooldown.";
  }
