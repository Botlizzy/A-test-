...
describe("auth confirmation redirect", () => {
  it("builds a deployed-origin confirmation URL without duplicate slashes", () => {
    expect(getConfirmationRedirect("https://streamvideo-h2f3nxnx.manus.space/")).toBe("https://elizzy-host.nx.kg/?confirmed=1");
    expect(getConfirmationRedirect("https://a-test-ten.vercel.app/")).toBe("https://elizzy-host.nx.kg/?confirmed=1");
  });

  it("never sends confirmation email links to local or sandbox origins", () => {
    expect(getConfirmationRedirect("http://localhost:3000")).toBe("https://elizzy-host.nx.kg/?confirmed=1");
    expect(getConfirmationRedirect("https://3000-example.us2.manus.computer")).toBe("https://elizzy-host.nx.kg/?confirmed=1");
    expect(getConfirmationRedirect("http://127.0.0.1:5173/")).toBe("https://elizzy-host.nx.kg/?confirmed=1");
  });
