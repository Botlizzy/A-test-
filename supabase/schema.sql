...
create table if not exists public.profiles (
  ...
  account_warning_started_at timestamptz,
...
alter table public.profiles add column if not exists account_warning_started_at timestamptz;
...
begin
  ...
  from auth.users where id = target_user_id
  on conflict (id) do nothing;
  update public.profiles set account_warning = flagged, account_warning_started_at = case when flagged then now() else null end where id = target_user_id returning * into updated_profile;
  if updated_profile.id is null then raise exception 'Customer not found'; end if;
  return updated_profile;
end;
$$;
...
returns table (id uuid, full_name text, email text, avatar_url text, account_status text, account_warning boolean, account_warning_started_at timestamptz, premium_active boolean, premium_expires_at timestamptz)
